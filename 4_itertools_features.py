...
def demonstrate_permutations():
    ...
    for item in permutation_result:
        logger.info(f"Перестановка: {item}")
...
def demonstrate_zip_longest():
    ...
    for item in zip_longest_result:
        logger.info(f"Комбинация: {item}")
...
def demonstrate_islice():
    ...
    islice_result = itertools.islice(count_numbers, 5, 15, 2)  # Срез от 5 до 15 с шагом 2
...
def demonstrate_combinations_with_replacement():
    ...
    elements = ['A', 'B']
    combinations_with_replacement_result = itertools.combinations_with_replacement(elements, 2)
